...
export default function Login({ theme, onLoginSuccess }: LoginProps) {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const envUsername = (import.meta as any).env?.VITE_GIS_PORTAL_USERNAME;
      const envPassword = (import.meta as any).env?.VITE_GIS_PORTAL_PASSWORD;

      // Determine final expected credentials with safe fallback to "admin" if not configured
      const isPlaceholderOrEmpty = (val: string | undefined) => {
        if (!val) return true;
        const clean = val.trim().toLowerCase();
        return clean === "" || clean.includes("enter") || clean.includes("username") || clean.includes("password") || clean.includes("your");
      };

      const finalExpectedUsername = isPlaceholderOrEmpty(envUsername) ? "admin" : envUsername;
      const finalExpectedPassword = isPlaceholderOrEmpty(envPassword) ? "admin" : envPassword;

      if (username === finalExpectedUsername && password === finalExpectedPassword) {
    ...
    <div className="absolute inset-0 z-[1000] bg-slate-950/25 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex flex-col items-center mb-6 text-center select-none">
          ...
          <h3 className={`text-xl font-bold tracking-tight ${theme === "dark" ? "text-white" : "text-slate-800"}`}>Almora Geoportal</h3>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full h-10 border focus:ring-1 text-sm pl-10 pr-4 py-2.5 leading-normal rounded-lg transition outline-none ${
                  theme === "dark"
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full h-10 border focus:ring-1 text-sm pl-10 pr-10 py-2.5 leading-normal rounded-lg transition outline-none ${
                  theme === "dark"
                    ? "bg-slate-950/50 border-slate-700/60 focus:border-indigo-500/80 focus:ring-indigo-500/80 text-white placeholder-slate-500"
                    : "bg-white/50 border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-slate-900 placeholder-slate-400"
                }`}
